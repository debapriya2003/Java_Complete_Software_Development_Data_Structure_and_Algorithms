```
└── 📁take_you_foreward_A2Z_Solutions
    └── 📁arrays
        └── 📁easy
            ├── CheckIfArrayIsSorted.java
            ├── FindMissingNumber.java
            ├── FindNumberAppearsOnceOthersTwice.java
            ├── FindTheUnion.java
            ├── LargestElementInArray.java
            ├── LeftRotateByDPlaces.java
            ├── LeftRotateByOnePlace.java
            ├── LinearSearch.java
            ├── LongestSubarraySumKPositivesNegatives.java
            ├── LongestSubarrayWithSumKPositives.java
            ├── MaximumConsecutiveOnes.java
            ├── MoveZerosToEnd.java
            ├── RemoveDuplicatesFromSortedArray.java
            ├── SecondLargestElementWithoutSorting.java
        └── 📁hard
            ├── CountInversions.java
            ├── CountSubarraysWithGivenXorK.java
            ├── FindRepeatingAndMissingNumber.java
            ├── FourSumProblem.java
            ├── LargestSubarrayWithZeroSum.java
            ├── MajorityElementN3.java
            ├── MaximumProductSubarray.java
            ├── MergeOverlappingSubintervals.java
            ├── MergeTwoSortedArraysWithoutExtraSpace.java
            ├── PascalsTriangle.java
            ├── ReversePairs.java
            ├── ThreeSumProblem.java
        └── 📁medium
            ├── AlternatingPositiveNegative.java
            ├── CountSubarraysWithGivenSum.java
            ├── KadanesAlgorithm.java
            ├── LeadersInArray.java
            ├── LongestConsecutiveSequence.java
            ├── MajorityElement.java
            ├── NextPermutation.java
            ├── PrintMatrixInSpiral.java
            ├── PrintSubarrayWithMaximumSum.java
            ├── RotateMatrixBy90Degrees.java
            ├── SetMatrixZeros.java
            ├── SortArrayOfZerosOnesAndTwos.java
            ├── StockBuyAndSell.java
            ├── TwoSumProblem.java
    └── 📁basics
        ├── Basics.java
        ├── collections.java
        ├── HashingBasics.java
        ├── Maths.java
        ├── patterns.java
        ├── Recursion.java
    └── 📁binarry_search
        └── 📁BS_on_1D
            ├── BinarySearchFind.java
            ├── CountArrayRotations.java
            ├── CountOccurrences.java
            ├── FindMinRotatedArray.java
            ├── FindPeakElement.java
            ├── FirstAndLastOccurrence.java
            ├── FloorAndCeil.java
            ├── LowerBound.java
            ├── SearchInsertPosition.java
            ├── SearchRotatedArrayI.java
            ├── SearchRotatedArrayII.java
            ├── SingleElement.java
            ├── UpperBound.java
        └── 📁BS_on_2D
            ├── FindPeakElement2D.java
            ├── MatrixMedian.java
            ├── RowWithMaxOnes.java
            ├── SearchIn2DMatrix.java
            ├── SearchRowColumnSorted.java
        └── 📁BS_on_Answers
            ├── AggressiveCows.java
            ├── BookAllocationProblem.java
            ├── CapacityToShip.java
            ├── FindNthRoot.java
            ├── FindSmallestDivisor.java
            ├── FindSquareRoot.java
            ├── KokoEatingBananas.java
            ├── KthElementTwoArrays.java
            ├── KthMissingPositive.java
            ├── MedianOfTwoSortedArrays.java
            ├── MinimizeMaxDistance.java
            ├── MinimumDaysToBouquets.java
            ├── PaintersPartition.java
            ├── SplitArrayLargestSum.java
    └── 📁binary_trees
        └── 📁hard
            ├── BurnBinaryTree.java
            ├── ChildrenSumProperty.java
            ├── ConstructTreeFromInorderPostorder.java
            ├── ConstructTreeFromInorderPreorder.java
            ├── CountNodesCompleteBinaryTree.java
            ├── FlattenBinaryTree.java
            ├── LCAInBinaryTree.java
            ├── MaximumWidthBinaryTree.java
            ├── MorrisInorderTraversal.java
            ├── MorrisPreorderTraversal.java
            ├── NodesAtDistanceK.java
            ├── RootToNodePath.java
            ├── SerializeDeserializeBinaryTree.java
            ├── uniquebtree.java
        └── 📁mid
            ├── BottomViewBinaryTree.java
            ├── BoundaryTraversal.java
            ├── CheckBalancedBinaryTree.java
            ├── DiameterOfBinaryTree.java
            ├── HeightOfBinaryTree.java
            ├── IdenticalBinaryTrees.java
            ├── LeftRightViewBinaryTree.java
            ├── MaximumPathSum.java
            ├── SymmetricBinaryTree.class
            ├── SymmetricBinaryTree.java
            ├── SymmetricBinaryTree$Node.class
            ├── TopViewBinaryTree.java
            ├── VerticalOrderTraversal.java
            ├── ZigZagTraversal.java
        └── 📁traversals
            ├── AllTraversalsInOne.java
            ├── BinaryTreeTraversals.java
            ├── BinaryTreeTraversalTypes.java
            ├── IntroductionToTrees.java
            ├── IterativeInorderTraversal.java
            ├── IterativePostorderOneStack.java
            ├── IterativePostorderTwoStacks.java
            ├── IterativePreorderTraversal.java
            ├── LevelOrderTraversal.java
    └── 📁bit_manipulation
        └── 📁advanced_maths
            ├── AllDivisors.java
            ├── Power.java
            ├── PrimeFactorisationSieve.java
            ├── PrimeFactors.java
            ├── SieveOfEratosthenes.java
        └── 📁interview_problems
            ├── CountBitsToFlip.java
            ├── OddOccurrence.java
            ├── PowerSet.java
            ├── TwoOddOccurrences.java
            ├── XorInRange.java
        └── 📁intro
            ├── BitManipulationTheory.java
            ├── CheckIfIthBitIsSet.java
            ├── CheckIfOdd.java
            ├── CheckIfPowerOfTwo.java
            ├── CountSetBits.java
            ├── DivideTwoIntegers.java
            ├── SetUnsetRightmostBit.java
            ├── SwapTwoNumbers.java
    └── 📁Dynamic_Programming
        └── 📁LIS
            ├── LargestDivisibleSubset.java
            ├── LongestBitonicSubsequence.java
            ├── LongestIncreasingSubsequenceDP.java
            ├── LongestIncreasingSubsequenceLength.java
            ├── LongestStringChain.java
            ├── NumberOfLongestIncreasingSubsequences.java
            ├── PrintLongestIncreasingSubsequence.java
        └── 📁MCMandPartition
            ├── BurstBalloons.java
            ├── EvaluateBooleanExpressionTrue.java
            ├── MatrixChainMultiplication.java
            ├── MatrixChainMultiplicationBottomUp.java
            ├── MinimumCostToCutStick.java
            ├── PalindromePartitioningII.java
            ├── PartitionArrayForMaximumSum.java
        └── 📁oneD
            ├── ClimbingStairs.java
            ├── FrogJump.java
            ├── FrogJumpK.java
            ├── HouseRobber.java
            ├── MaxNonAdjacentSum.java
        └── 📁Squares
            ├── CountSquareSubmatricesWithAllOnes.java
            ├── MaximumRectangleAllOnes.java
        └── 📁stocks
            ├── BestTimeToBuyAndSellStock.java
            ├── BestTimeToBuyAndSellStockII.java
            ├── BestTimeToBuyAndSellStockIII.java
            ├── BestTimeToBuyAndSellStockIV.java
            ├── BestTimeToBuyAndSellStockWithCooldown.java
            ├── BestTimeToBuyAndSellStockWithFee.java
        └── 📁strings
            ├── DistinctSubsequences.java
            ├── EditDistance.java
            ├── LongestCommonSubsequence.java
            ├── LongestCommonSubstring.java
            ├── LongestPalindromicSubsequence.java
            ├── MinimumInsertionsToMakePalindrome.java
            ├── MinInsertionsDeletionsConvertString.java
            ├── PrintLongestCommonSubsequence.java
            ├── ShortestCommonSupersequence.java
            ├── WildcardMatching.java
        └── 📁subsequences
            ├── AssignCookies.java
            ├── CoinChange2.java
            ├── CountPartitionsWithGivenDifference.java
            ├── CountSubsetsWithSumK.java
            ├── MinimumCoins.java
            ├── PartitionEqualSubsetSum.java
            ├── PartitionSetMinAbsDiff.java
            ├── RodCuttingProblem.java
            ├── SubsetSumEqualToTarget.java
            ├── TargetSum.java
            ├── UnboundedKnapsack.java
        └── 📁twoD3Dandgrid
            ├── FallingPathSum.java
            ├── GridUniquePaths.java
            ├── GridUniquePathsII.java
            ├── MinimumPathSum.java
            ├── MinimumPathSumTriangle.java
            ├── NinjaAndFriends.java
            ├── NinjaTraining.java
    └── 📁Graphs
        └── 📁BFSandDFS
            ├── BipartiteGraphDFS.java
            ├── ConnectedComponentsMatrix.java
            ├── CycleDetectionDirectedDFS.java
            ├── CycleDetectionUndirectedBFS.java
            ├── CycleDetectionUndirectedDFS.java
            ├── FloodFill.java
            ├── NumberOfDistinctIslands.java
            ├── NumberOfEnclaves.java
            ├── NumberOfProvinces.java
            ├── RottenOranges.java
            ├── SurroundedRegions.java
            ├── WordLadder1.java
            ├── WordLadder2.java
            ├── ZeroOneMatrix.java
        └── 📁Learning
            ├── BFS.java
            ├── ConnectedComponents.java
            ├── DFS.java
            ├── GraphRepresentation.java
            ├── GraphTheoryExample.java
        └── 📁MSTandDisSet
            ├── AccountsMerge.java
            ├── DisjointSetUnionByRank.java
            ├── DisjointSetUnionBySize.java
            ├── KruskalAlgorithm.java
            ├── MakeNetworkConnected.java
            ├── MakingALargeIsland.java
            ├── MinimumSpanningTree.java
            ├── MostStonesRemoved.java
            ├── NumberOfIslandsII.java
            ├── PrimsAlgorithm.java
            ├── SwimInRisingWater.java
        └── 📁others
            ├── ArticulationPoint.java
            ├── BridgesInGraph.java
            ├── KosarajuAlgorithm.java
        └── 📁ShortestPathAlgo_Problems
            ├── BellmanFord.java
            ├── BinaryMazeShortestPath.java
            ├── CheapestFlightsWithinKStops.java
            ├── Dijkstra.java
            ├── DijkstraWhyPriorityQueue.java
            ├── FindCityWithSmallestNeighborsThreshold.java
            ├── FloydWarshall.java
            ├── MinMultiplicationsModulo.java
            ├── NetworkDelayTime.java
            ├── NumberOfWaysToArriveAtDestination.java
            ├── PathWithMinimumEffort.java
            ├── ShortestPathInDAG.java
            ├── ShortestPathUGUnitWeights.java
        └── 📁Topological_sort_problems
            ├── AlienDictionary.java
            ├── CourseScheduleI.java
            ├── CourseScheduleII.java
            ├── CycleDetectionDirectedBFS.java
            ├── FindEventualSafeStates.java
            ├── KahnAlgorithm.java
            ├── TopologicalSortDFS.java
    └── 📁greedy_algorithms
        └── 📁easy
            ├── AssignCookies.java
            ├── FractionalKnapsack.java
            ├── LemonadeChange.java
            ├── MinimumCoins.java
            ├── ValidParenthesis.java
        └── 📁medium
            ├── Candy.java
            ├── InsertInterval.java
            ├── JobSequencing.java
            ├── JumpGame.java
            ├── JumpGame2.java
            ├── LRUPageReplacement.java
            ├── MergeIntervals.java
            ├── MinimumPlatforms.java
            ├── NMeetingsOneRoom.java
            ├── NonOverlappingIntervals.java
            ├── ShortestJobFirst.java
    └── 📁heap
        └── 📁hard
            ├── ConnectNRopes.java
            ├── DesignTwitter.java
            ├── FindMedianFromDataStream.java
            ├── KMostFrequentElements.java
            ├── KthLargestInStream.java
            ├── MaximumSumCombination.java
        └── 📁intro
            ├── CheckIfMinHeap.java
            ├── ConvertMinHeapToMaxHeap.java
            ├── IntroductionToPriorityQueue.java
            ├── MinHeapAndMaxHeapImplementation.java
        └── 📁medium
            ├── HandOfStraights.java
            ├── KthLargestElement.java
            ├── KthSmallestElement.java
            ├── MergeMSortedLists.java
            ├── ReplaceByRank.java
            ├── SortKSortedArray.java
            ├── TaskScheduler.java
    └── 📁important_sorting
        ├── BubbleSort.java
        ├── InsertionSort.java
        ├── MergeSort.java
        ├── QuickSort.java
        ├── RecursiveBubbleSort.java
        ├── RecursiveInsertionSort.java
        ├── SelectionSort.java
    └── 📁linked_lists
        └── 📁doubly_linked_list
            ├── DeletingNodeInDLL.java
            ├── InsertingNodeInDLL.java
            ├── IntroductionToDLL.java
            ├── ReverseDLL.java
        └── 📁hard_LL
            ├── CloneLinkedListWithRandomPointer.java
            ├── FlatteningOfLinkedList.java
            ├── ReverseLinkedListInGroupsOfK.java
            ├── RotateLinkedList.java
        └── 📁medium_DLL
            ├── DeleteAllOccurrencesOfKeyInDLL.java
            ├── FindPairsWithGivenSumInDLL.java
            ├── RemoveDuplicatesFromSortedDLL.java
        └── 📁medium_LL
            ├── AddOneToNumberInLinkedList.java
            ├── AddTwoNumbersInLinkedList.java
            ├── CheckIfLinkedListIsPalindrome.java
            ├── DeleteMiddleNode.java
            ├── DetectLoopInLinkedList.java
            ├── FindIntersectionPointOfYLinkedList.java
            ├── FindLoopStartingPoint.java
            ├── LengthOfLoopInLinkedList.java
            ├── MiddleOfLinkedList.java
            ├── RemoveNthNodeFromBack.java
            ├── ReverseLinkedListIterative.java
            ├── ReverseLinkedListRecursive.java
            ├── SegregateOddEvenNodes.java
            ├── SortLinkedList.java
            ├── SortZerosOnesAndTwos.java
        └── 📁singly_linked_list
            ├── DeletingNodeInLinkedList.java
            ├── FindLengthOfLinkedList.java
            ├── InsertingNodeInLinkedList.java
            ├── IntroductionToLinkedList.java
            ├── SearchElementInLinkedList.java
    └── 📁recursion_patternwise
        └── 📁all_combo_hard
            ├── AndroidUnlockPatterns.java
            ├── ExpressionAddOperators.java
            ├── MColoringGraph.java
            ├── NQueens.java
            ├── PalindromePartitioning.java
            ├── RatInMaze.java
            ├── SudokuSolver.java
            ├── WordBreak.java
            ├── WordSearch.java
        └── 📁strong_hold
            ├── CountGoodNumbers.java
            ├── PowXN.java
            ├── RecursiveImplementationOfAtoi.java
            ├── ReverseStackUsingRecursion.java
            ├── SortStackUsingRecursion.java
        └── 📁subsequences
            ├── CheckIfSubsequenceWithSumK.java
            ├── CombinationSum.java
            ├── CombinationSumII.java
            ├── CombinationSumIII.java
            ├── CountSubsequencesWithSumK.java
            ├── GenerateAllBinaryStrings.java
            ├── GenerateParenthesis.java
            ├── LetterCombinationsOfPhoneNumber.java
            ├── PrintAllSubsequences.java
            ├── SubsequencePatternTheory.java
            ├── SubsetSumI.java
            ├── SubsetSumII.java
    └── 📁sliding_window_two_pointer
        └── 📁hard
            ├── LongestSubstringWithAtMostKDistinctCharacters.java
            ├── MinimumWindowSubsequence.java
            ├── MinimumWindowSubstring.java
            ├── SubarrayWithKDifferentIntegers.java
        └── 📁medium
            ├── BinarySubarrayWithSum.java
            ├── CountNumberOfNiceSubarrays.java
            ├── FruitIntoBasskets.java
            ├── LongestRepeatingCharacterReplacement.java
            ├── LongestSubstringWithoutRepeatingCharacters.java
            ├── MaxConsecutiveOnes.java
            ├── MaximumPointsYouCanObtainFromCards.java
            ├── NumberOfSubstringsContainingAllThreeCharacters.java
    └── 📁stacks_queues
        └── 📁implementation_problems
            ├── LFUCache.java
            ├── LRUCache.java
            ├── SlidingWindowMaximum.java
            ├── StockSpanProblem.java
            ├── TheCelebrityProblem.java
        └── 📁intro
            ├── BalancedParenthesis.java
            ├── MinStack.java
            ├── QueueArray.java
            ├── QueueLinkedList.java
            ├── QueueUsingStack.java
            ├── StackArray.java
            ├── StackLinkedList.java
            ├── StackUsingQueue.java
        └── 📁monotonic_stack
            ├── AsteroidCollision.java
            ├── LargestRectangleInHistogram.java
            ├── MaximalRectangles.java
            ├── NextGreaterElement.java
            ├── NextGreaterElement2.java
            ├── NextSmallerElement.java
            ├── NumberOfNGES.java
            ├── RemoveKDigits.java
            ├── SumOfSubarrayMinimums.java
            ├── SumOfSubarrayRanges.java
            ├── TrappingRainwater.java
        └── 📁PrefixInfixPostFixConversionProblems
            ├── InfixToPostfix.java
            ├── InfixToPrefix.java
            ├── PostfixToInfix.java
            ├── PostfixToPrefix.java
            ├── PrefixToInfix.java
            ├── PrefixToPostfix.java
    └── 📁strings
        └── 📁basics
            ├── AnagramCheck.java
            ├── IsomorphicStringCheck.java
            ├── LargestOddNumberInString.java
            ├── LongestCommonPrefix.java
            ├── PalindromeCheck.java
            ├── RemoveOutermostParenthesis.java
            ├── ReverseWordsInString.java
            ├── StringRotationCheck.java
        └── 📁hard
            ├── CountAndSay.java
            ├── CountPalindromicSubsequence.java
            ├── HashingInStringsTheory.java
            ├── KMPAlgorithm.java
            ├── LongestHappyPrefix.java
            ├── MinimumBracketReversals.java
            ├── RabinKarp.java
            ├── ShortestPalindrome.java
            ├── ZFunction.java
        └── 📁medium
            ├── CountNumberOfSubstrings.java
            ├── ImplementAtoi.java
            ├── LongestPalindromicSubstring.java
            ├── MaximumNestingDepthOfParentheses.java
            ├── ReverseEveryWordInAString.java
            ├── RomanNumberConversion.java
            ├── SortCharactersByFrequency.java
            ├── SumOfBeautyOfAllSubstrings.java
    └── 📁Tries
        ├── BitPrerequisitesForTrie.java
        ├── LongestStringWithAllPrefixes.java
        ├── MaximumXORInArray.java
        ├── MaximumXORWithElement.java
        ├── NumberOfDistinctSubstrings.java
        ├── Trie.java
        └── TrieII.java
```